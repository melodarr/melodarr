...
import styles from './TrackFileEditorModalContent.module.css';
...
class TrackFileEditorModalContent extends Component {
  ...
  render() {
    ...
    const qualityOptions = _.reduceRight(
      qualities,
      (acc, quality) => {
        acc.push({
          key: quality.id,
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
        <ModalHeader>Manage Tracks</ModalHeader>

        <ModalBody>
          {isFetching && !isPopulated ? <LoadingIndicator /> : null}

          {!isFetching && error ? <div>{error}</div> : null}

          {isPopulated && !items.length ? (
            <div>No track files to manage.</div>
          ) : null}
