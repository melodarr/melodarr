...
function createMapStateToProps() {
  return createSelector(
    ...
    (albumId, tracks, trackFiles, schema, artist) => {
      ...
      const sorted = _.orderBy(
        filtered,
        ['albumId', 'mediumNumber', 'absoluteTrackNumber'],
        ['desc', 'asc', 'asc']
      );
...
function createMapDispatchToProps(dispatch, _props) {
  return {
    ...
    dispatchFetchQualityProfileSchema(_name, _path) {
...
class TrackFileEditorModalContentConnector extends Component {
  ...
  render() {
    const {
      _dispatchFetchQualityProfileSchema,
      _dispatchUpdateTrackFiles,
      _dispatchFetchTracks,
      _dispatchClearTracks,
      ...otherProps
    } = this.props;
