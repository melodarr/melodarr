...
class CalendarOptionsModalContent extends Component {
  ...
  onOptionInputChange = ({ name, value }) => {
    const { dispatchSetCalendarOption } = this.props;

    dispatchSetCalendarOption({ [name]: value });
  };

  onGlobalInputChange = ({ name, value }) => {
    const { dispatchSaveUISettings } = this.props;
  ...
  render() {
    const { collapseMultipleAlbums, showCutoffUnmetIcon, onModalClose } =
      this.props;
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
        <ModalHeader>Calendar Options</ModalHeader>

        <ModalBody>
          <FieldSet legend={translate('Local')}>
            <Form>
              <FormGroup>
                <FormLabel>{translate('CollapseMultipleAlbums')}</FormLabel>
            ...
            <Form>
              ...
              <FormGroup>
                ...
                <FormInputGroup
                  ...
                  onChange={this.onGlobalInputChange}
                  helpText={translate(
                    'ShownAboveEachColumnWhenWeekIsTheActiveView'
                  )}
                />
              </FormGroup>
