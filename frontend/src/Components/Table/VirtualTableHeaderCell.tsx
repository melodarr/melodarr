...
export function headerRenderer(headerProps) {
  const { columnData = {}, dataKey, label } = headerProps;

  return (
    // eslint-disable-next-line no-use-before-define
    <VirtualTableHeaderCell name={dataKey} {...columnData}>
      {label}
...
class VirtualTableHeaderCell extends Component {
  ...
  onPress = () => {
    const { name, fixedSortDirection } = this.props;
  ...
  render() {
    const {
      ...
      sortKey,
      sortDirection,
      fixedSortDirection: _fixedSortDirection,
      children,
