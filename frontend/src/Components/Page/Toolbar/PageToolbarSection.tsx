...
function calculateOverflowItems(children, isMeasured, width, collapseButtons) {
  ...
  validChildren.forEach((child, _index) => {
...
class PageToolbarSection extends Component {
  ...
  render() {
    const { children, alignContent, collapseButtons } = this.props;

    const { isMeasured, width } = this.state;

    const { buttons, buttonCount, overflowItems } = calculateOverflowItems(
      children,
      isMeasured,
      width,
      collapseButtons
