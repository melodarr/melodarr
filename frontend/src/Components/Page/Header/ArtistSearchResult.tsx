...
import styles from './ArtistSearchResult.module.css';

function ArtistSearchResult(props) {
  const { match, artistName, images, foreignArtistId, tags } = props;
  ...
  return (
    <div className={styles.result}>
      ...
      <div className={styles.titles}>
        <div className={styles.title}>{artistName}</div>

        {match.key === 'foreignArtistId' && foreignArtistId ? (
          <div className={styles.alternateTitle}>MbId: {foreignArtistId}</div>
        ) : null}
