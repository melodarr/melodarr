...
import FuseWorker from './fuse.worker.js?worker';
import styles from './ArtistSearchInput.module.css';
...
class ArtistSearchInput extends Component {
  ...
  renderSuggestion(item, { query }) {
    if (item.type === ADD_NEW_TYPE) {
      return (
        <div className={styles.addNewArtistSuggestion}>Search for {query}</div>
      );
    }

    return <ArtistSearchResult {...item.item} match={item.matches[0]} />;
  }
  ...
  render() {
    const { value, loading, suggestions } = this.state;
