...
class Menu extends Component {
  ...
  render() {
    const { className, children, alignMenu } = this.props;

    const { maxHeight, isMenuOpen } = this.state;
    ...
    return (
      <Manager>
        <Reference>
          {({ ref }) => (
            <div ref={ref} id={this._menuButtonId} className={className}>
              {button}
            </div>
          )}
        </Reference>
        ...
        <Portal>
          <Popper {...popperOptions[alignMenu]}>
            {({ ref, style, scheduleUpdate }) => {
              ...
              return React.cloneElement(childrenArray[1], {
                forwardedRef: ref,
                style: {
                  ...style,
                  maxHeight,
