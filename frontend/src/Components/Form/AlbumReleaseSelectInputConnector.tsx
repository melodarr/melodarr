    ...
    (albumReleases) => {
      const values = _.map(albumReleases.value, (albumRelease) => {
        ...
        return {
          ...
          value:
            `${title}` +
            `${disambiguation ? ' (' : ''}${titleCase(disambiguation)}${
              disambiguation ? ')' : ''
            }` +
      ...
      const value = _.find(albumReleases.value, {
        monitored: true,
      }).foreignReleaseId;
...
class AlbumReleaseSelectInputConnector extends Component {
  ...
  onChange = ({ name, value }) => {
    const { albumReleases } = this.props;

    const updatedReleases = _.map(albumReleases.value, (e) => ({
      ...e,
      monitored: false,
    }));
  ...
  render() {
    return <SelectInput {...this.props} onChange={this.onChange} />;
  }
}
