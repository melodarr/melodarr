...
function createMapStateToProps() {
  return createSelector(
    ...
    (
      ...
      const profiles = metadataProfiles.items.filter(
        (item) => item.name !== metadataProfileNames.NONE
      );
      const noneProfile = metadataProfiles.items.find(
        (item) => item.name === metadataProfileNames.NONE
      );
...
class MetadataProfileSelectInputConnector extends Component {
  ...
  componentDidMount() {
    ...
    if (
      !value ||
      !values.some(
        (option) => option.key === value || parseInt(option.key) === value
      )
    ) {
  ...
  onChange = ({ name, value }) => {
    this.props.onChange({
      name,
      value: value === 'noChange' ? value : parseInt(value),
    });
  };
  ...
  render() {
    return <EnhancedSelectInput {...this.props} onChange={this.onChange} />;
  }
}
