...
class EnhancedSelectInput extends Component {
    ...
    if (Array.isArray(this.props.value)) {
      ...
      if (index === -1) {
        newValue = this.props.values
          .map((v) => v.key)
          .filter((v) => v === value || this.props.value.includes(v));
      } else {
  ...
  render() {
    ...
    const { selectedIndex, width, isOpen, isMobile } = this.state;
                      ...
                      >
                        {values.map((v, index) => {
                          ...
                          const parentSelected =
                            hasParent && value.includes(v.parentKey);
                          return (
                            <OptionComponent
                              key={v.key}
                              id={v.key}
...
EnhancedSelectInput.propTypes = {
  ...
  selectedValueComponent: PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.func,
  ]).isRequired,
