...
function createMapStateToProps() {
  return createSelector(
    ...
    (playlists) => {
      const { items, ...otherState } = playlists;
...
class PlaylistInputConnector extends Component {
  ...
  _populate() {
    const { provider, providerData, dispatchFetchOptions } = this.props;
  ...
  _getAccessToken(props) {
    return _.filter(props.providerData.fields, { name: 'accessToken' })[0]
      .value;
  }
  ...
  render() {
    return (
      <PlaylistInput {...this.props} onRefreshPress={this.onRefreshPress} />
    );
  }
}
