...
const importantFieldNames = ['baseUrl', 'apiPath', 'apiKey'];
...
function createMapStateToProps() {
  return createSelector(
    (state, { selectOptionsProviderAction }) =>
      state.providerOptions[selectOptionsProviderAction] || defaultState,
    (options) => {
      if (options) {
        return {
...
class EnhancedSelectInputConnector extends Component {
  ...
  _cleanup() {
    const { selectOptionsProviderAction, dispatchClearOptions } = this.props;
  ...
  render() {
    return <EnhancedSelectInput {...this.props} onOpen={this.onOpen} />;
  }
}
