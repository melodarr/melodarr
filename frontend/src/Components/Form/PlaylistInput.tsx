...
class PlaylistInput extends Component {
  ...
  render() {
    const { className, items, user, isFetching, isPopulated } = this.props;
    ...
    return (
      <div className={className}>
        ...
        {!isPopulated && !isFetching && (
          <div>Authenticate with spotify to retrieve playlists to import.</div>
        )}
        ...
        {isPopulated && !isFetching && user && !items.length && (
          <div>No playlists found for Spotify user {user}.</div>
        )}
