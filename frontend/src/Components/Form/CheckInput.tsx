...
class CheckInput extends Component {
  ...
  toggleChecked = (checked, shiftKey) => {
    const { name, value, checkedValue, uncheckedValue } = this.props;
  ...
  render() {
    ...
    return (
      <div className={containerClassName}>
        <label className={styles.label} onClick={this.onClick}>
          <input
            ref={this.setRef}
          ...
          >
            {isChecked && <Icon name={icons.CHECK} />}

            {isIndeterminate && <Icon name={icons.CHECK_INDETERMINATE} />}
          </div>
