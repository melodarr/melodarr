...
class OrganizePreviewModalContent extends Component {
  ...
  render() {
    const { isFetching, isPopulated, error, items, trackFormat, onModalClose } =
      this.props;
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
        <ModalHeader>Organize & Rename</ModalHeader>
        ...
        <ModalBody>
          ...
          {!isFetching && isPopulated && !items.length && (
            <div>{translate('SuccessMyWorkIsDoneNoFilesToRename')}</div>
          )}
          ...
          {!isFetching && isPopulated && !!items.length && (
            <div>
              <Alert>
                <div>
                  Naming pattern:
                  <span className={styles.trackFormat}>{trackFormat}</span>
                </div>
              </Alert>
        ...
        <ModalFooter>
          ...
          <Button kind={kinds.PRIMARY} onPress={this.onOrganizePress}>
            {translate('Organize')}
          </Button>
        </ModalFooter>
      </ModalContent>
