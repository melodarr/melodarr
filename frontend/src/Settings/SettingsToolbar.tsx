...
class SettingsToolbar extends Component {
  ...
  componentDidMount() {
    this.props.bindShortcut(shortcuts.SAVE_SETTINGS.key, this.saveSettings, {
      isGlobal: true,
    });
  }
  ...
  saveSettings = (event) => {
    ...
    const { hasPendingChanges, onSavePress } = this.props;
        ...
        <PageToolbarSection>
          ...
          {showSave && (
            <PageToolbarButton
              label={
                hasPendingChanges
                  ? translate('HasPendingChangesSaveChanges')
                  : translate('HasPendingChangesNoChanges')
              }
              iconName={icons.SAVE}
              isSpinning={isSaving}
