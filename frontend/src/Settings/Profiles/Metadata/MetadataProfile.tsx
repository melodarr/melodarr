...
import styles from './MetadataProfile.module.css';
...
class MetadataProfile extends Component {
  ...
  onCloneMetadataProfilePress = () => {
    const { id, onCloneMetadataProfilePress } = this.props;
  ...
  render() {
    const { id, name, primaryAlbumTypes, secondaryAlbumTypes, isDeleting } =
      this.props;
      ...
      >
        ...
        <div className={styles.albumTypes}>
          {primaryAlbumTypes.map((item) => {
            ...
            return (
              <Label key={item.albumType.id} kind={kinds.DEFAULT} title={null}>
                {item.albumType.name}
              </Label>
            );
          })}
        ...
        <div className={styles.albumTypes}>
          {secondaryAlbumTypes.map((item) => {
            ...
            return (
              <Label key={item.albumType.id} kind={kinds.INFO} title={null}>
                {item.albumType.name}
              </Label>
            );
          })}
