...
class PrimaryTypeItem extends Component {
  ...
  onAllowedChange = ({ value }) => {
    const { albumTypeId, onMetadataPrimaryTypeItemAllowedChange } = this.props;
  ...
  render() {
    const { name, allowed } = this.props;

    return (
      <div className={classNames(styles.metadataProfileItem)}>
        <label className={styles.albumTypeName}>
          <CheckInput
