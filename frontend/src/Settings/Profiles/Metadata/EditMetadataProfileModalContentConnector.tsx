  ...
  return createSelector(
    ...
    (metadataProfile) => {
      ...
      return _.reduceRight(
        primaryAlbumTypes.value,
        (result, { allowed, albumType }) => {
          if (allowed) {
            result.push({
              key: albumType.id,
...
class EditMetadataProfileModalContentConnector extends Component {
  ...
  onMetadataPrimaryTypeItemAllowedChange = (id, allowed) => {
    ...
    const item = _.find(
      metadataProfile.primaryAlbumTypes.value,
      (i) => i.albumType.id === id
    );
  ...
  onMetadataSecondaryTypeItemAllowedChange = (id, allowed) => {
    ...
    const item = _.find(
      metadataProfile.secondaryAlbumTypes.value,
      (i) => i.albumType.id === id
    );
    item.allowed = allowed;
  ...
  onMetadataReleaseStatusItemAllowedChange = (id, allowed) => {
    ...
    const item = _.find(
      metadataProfile.releaseStatuses.value,
      (i) => i.releaseStatus.id === id
    );
    item.allowed = allowed;
  ...
  render() {
    if (
      _.isEmpty(this.props.item.primaryAlbumTypes) &&
    ...
    return (
      <EditMetadataProfileModalContent
        ...
        onMetadataPrimaryTypeItemAllowedChange={
          this.onMetadataPrimaryTypeItemAllowedChange
        }
        onMetadataSecondaryTypeItemAllowedChange={
          this.onMetadataSecondaryTypeItemAllowedChange
        }
        onMetadataReleaseStatusItemAllowedChange={
          this.onMetadataReleaseStatusItemAllowedChange
        }
      />
    );
  }
