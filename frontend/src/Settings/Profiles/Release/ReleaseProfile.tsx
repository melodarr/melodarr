...
import styles from './ReleaseProfile.module.css';
...
class ReleaseProfile extends Component {
  ...
  render() {
    ...
    const { isEditReleaseProfileModalOpen, isDeleteReleaseProfileModalOpen } =
      this.state;
      ...
      >
        <div>
          {required.map((item) => {
            ...
            return (
              <Label className={styles.label} key={item} kind={kinds.SUCCESS}>
                <span title={item}>
                  {item.length > 20
        ...
        <div>
          {ignored.map((item) => {
            ...
            return (
              <Label className={styles.label} key={item} kind={kinds.DANGER}>
                <span title={item}>
                  {item.length > 20
                    ? `${item.slice(0, 10)}...${item.slice(-10)}`
                    : item}
