...
class QualityDefinition extends Component {
  ...
  trackRenderer(props, _state) {
    return <div {...props} className={styles.track} />;
  }

  thumbRenderer(props, _state) {
  ...
  render() {
    ...
    const preferredRate = preferredBytes
      ? `${formatBytes(preferredBytes, true)}/s`
      : translate('Unlimited');

    const maxBytes = maxSize && maxSize * 128;
    const maxRate = maxBytes
      ? `${formatBytes(maxBytes, true)}/s`
      : translate('Unlimited');
      ...
      <div className={styles.qualityDefinition}>
        ...
        <div className={styles.sizeLimit}>
          ...
          <div className={styles.sizes}>
            <div>
              <Popover
                anchor={<Label kind={kinds.INFO}>{minRate}</Label>}
            ...
            <div>
              <Popover
                anchor={<Label kind={kinds.SUCCESS}>{preferredRate}</Label>}
            ...
            <div>
              <Popover
                anchor={<Label kind={kinds.WARNING}>{maxRate}</Label>}
