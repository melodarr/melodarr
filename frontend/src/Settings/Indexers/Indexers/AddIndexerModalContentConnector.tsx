...
function createMapStateToProps() {
  return createSelector(
    ...
    (indexers) => {
      ...
      const usenetIndexers = _.filter(schema, {
        protocol: 'UsenetDownloadProtocol',
      });
      const torrentIndexers = _.filter(schema, {
        protocol: 'TorrentDownloadProtocol',
      });
      const otherIndexers = _.filter(
        schema,
        (x) =>
          x.protocol !== 'UsenetDownloadProtocol' &&
...
class AddIndexerModalContentConnector extends Component {
  ...
  onIndexerSelect = ({ implementation, implementationName, name }) => {
    this.props.selectIndexerSchema({
      implementation,
      implementationName,
      presetName: name,
    });
