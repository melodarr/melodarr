...
import styles from './TagDetailsDelayProfile.module.css';

function TagDetailsDelayProfile(props) {
  const { name: profileName, items } = props;
  ...
  return (
    <div className={styles.delayProfile}>
      ...
      <div className={styles.tags}>
        {items.map((item) => {
          const { protocol, name, allowed } = item;

          return (
            <DelayProfileItem key={protocol} name={name} allowed={allowed} />
          );
        })}
      </div>
    </div>
