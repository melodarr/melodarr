...
function TagDetailsModalContent(props) {
  ...
  return (
    <ModalContent onModalClose={onModalClose}>
      <ModalHeader>Tag Details - {label}</ModalHeader>
      ...
      <ModalBody>
        ...
        {artist.length ? (
          <FieldSet legend={translate('Artists')}>
            {artist.map((item) => {
              return <div key={item.id}>{item.artistName}</div>;
            })}
          </FieldSet>
        ) : null}
        ...
        {delayProfiles.length ? (
          <FieldSet legend={translate('DelayProfile')}>
            {delayProfiles.map((item) => {
              ...
              return (
                <TagDetailsDelayProfile key={id} name={name} items={items} />
              );
            })}
          </FieldSet>
        ) : null}
      ...
      <ModalFooter>
        {
          <Button
            ...
            title={
              isTagUsed
                ? translate('IsTagUsedCannotBeDeletedWhileInUse')
                : undefined
            }
