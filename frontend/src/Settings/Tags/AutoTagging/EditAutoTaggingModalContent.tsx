...
export default function EditAutoTaggingModalContent(props) {
  const { id, tagsFromId, onModalClose, onDeleteAutoTaggingPress } = props;
  ...
  const onAddSpecificationModalClose = useCallback(
  ...
  const onCloneSpecificationPress = useCallback(
    (specId) => {
  ...
  const onConfirmDeleteSpecification = useCallback(
    (specId) => {
    ...
    <ModalContent onModalClose={onModalClose}>
      ...
      <ModalBody>
        <div>
          ...
          {!isFetching && !!error ? (
            <div>{translate('AddAutoTagError')}</div>
          ) : null}
