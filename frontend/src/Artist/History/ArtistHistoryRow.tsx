...
import styles from './ArtistHistoryRow.module.css';
...
class ArtistHistoryRow extends Component {
  ...
  render() {
    ...
    const { isMarkAsFailedModalOpen } = this.state;

    return (
      <TableRow>
        <HistoryEventTypeCell eventType={eventType} data={data} />

        <TableRowCell key={name}>{album.title}</TableRowCell>
        ...
        <RelativeDateCellConnector date={date} />
