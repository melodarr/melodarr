...
import styles from './ArtistDetailsSeason.module.css';
...
function getAlbumStatistics(albums) {
  ...
  albums.forEach(({ monitored, releaseDate, statistics = {} }) => {
    ...
    const hasFiles =
      albumTrackFileCount > 0 && albumTrackFileCount === albumTotalTrackCount;
...
class ArtistDetailsSeason extends Component {
  ...
  _expandByDefault() {
    const { name, onExpandPress, items, uiSettings } = this.props;
    ...
    const expand = _.some(
      ...
      (item) =>
        ...
        (item.albumType === 'Broadcast' &&
          uiSettings.expandBroadcastByDefault) ||
  ...
  onExpandPress = () => {
    const { name, isExpanded } = this.props;
  ...
  onMonitorAlbumsPress = (monitored, { shiftKey: _shiftKey }) => {
  ...
  render() {
    ...
    const { isOrganizeModalOpen, isManageTracksOpen } = this.state;
      ...
      <div className={styles.albumType}>
        <div className={styles.header}>
          <div className={styles.left}>
            ...
            />
            <span className={styles.albumTypeLabel}>{label}</span>
            <Popover
          ...
          <Link className={styles.expandButton} onPress={this.onExpandPress}>
            <Icon
              ...
              title={
                isExpanded
                  ? translate('IsExpandedHideAlbums')
                  : translate('IsExpandedShowAlbums')
              }
              size={24}
            />
          ...
          {isExpanded && (
            <div className={styles.albums}>
              {items.length ? (
                ...
                <div className={styles.noAlbums}>No releases in this group</div>
              )}
