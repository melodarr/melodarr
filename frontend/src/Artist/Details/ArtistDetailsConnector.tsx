...
import {
  clearTrackFiles,
  fetchTrackFiles,
} from 'Store/Actions/trackFileActions';
...
const selectAlbums = createSelector(
  (state) => state.albums,
  (albums) => {
    const { items, isFetching, isPopulated, error } = albums;
...
const selectTrackFiles = createSelector(
  ...
  (trackFiles) => {
    const { items, isFetching, isPopulated, error } = trackFiles;
  ...
  return createSelector(
    ...
    (foreignArtistId, albums, trackFiles, allArtists, commands) => {
      ...
      const previousArtist =
        sortedArtist[artistIndex - 1] || _.last(sortedArtist);
      const nextArtist = sortedArtist[artistIndex + 1] || _.first(sortedArtist);
      const isArtistRefreshing = isCommandExecuting(
        findCommand(commands, {
          name: commandNames.REFRESH_ARTIST,
          artistId: artist.id,
        })
      );
      const artistRefreshingCommand = findCommand(commands, {
        name: commandNames.REFRESH_ARTIST,
      });
      ...
      const isSearching = isCommandExecuting(
        findCommand(commands, {
      ...
      const isRenamingArtistCommand = findCommand(commands, {
      ...
      const alternateTitles = _.reduce(
