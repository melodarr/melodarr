...
function createMapStateToProps() {
  return createSelector(
    ...
    (match, artist) => {
      ...
      const { isFetching, isPopulated, error, items } = artist;
...
class ArtistDetailsPageConnector extends Component {
  ...
  componentDidUpdate(_prevProps) {
    if (!this.props.foreignArtistId) {
      this.props.push(`${window.Melodarr.urlBase}/`);
      return;
    }
  }
  ...
  render() {
    const { foreignArtistId, isFetching, isPopulated, error } = this.props;
    ...
    if (!foreignArtistId) {
      return <NotFound message={translate('SorryThatArtistCannotBeFound')} />;
    }

    return <ArtistDetailsConnector foreignArtistId={foreignArtistId} />;
  }
}
...
ArtistDetailsPageConnector.propTypes = {
  ...
  match: PropTypes.shape({
    params: PropTypes.shape({ foreignArtistId: PropTypes.string.isRequired })
      .isRequired,
  }).isRequired,
