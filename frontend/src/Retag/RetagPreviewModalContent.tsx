...
import styles from './RetagPreviewModalContent.module.css';
...
class RetagPreviewModalContent extends Component {
  ...
  render() {
    const { isFetching, isPopulated, error, items, onModalClose } = this.props;
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
        <ModalHeader>{translate('WriteMetadataTags')}</ModalHeader>
        ...
        <ModalBody>
          ...
          {!isFetching && isPopulated && !items.length && (
            <div>{translate('SuccessMyWorkIsDoneNoFilesToRetag')}</div>
          )}
          ...
          {!isFetching && isPopulated && !!items.length && (
            <div>
              <Alert>
                <div>
                  MusicBrainz identifiers will also be added to the files; these
                  are not shown below.
                </div>
              </Alert>
        ...
        <ModalFooter>
          ...
          <Button kind={kinds.PRIMARY} onPress={this.onRetagPress}>
            {translate('Retag')}
          </Button>
        </ModalFooter>
      </ModalContent>
