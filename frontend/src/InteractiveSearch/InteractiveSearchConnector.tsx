...
class InteractiveSearchConnector extends Component {
  ...
  componentDidMount() {
    const { searchPayload, isPopulated, dispatchFetchReleases } = this.props;
  ...
  render() {
    const { dispatchFetchReleases: _dispatchFetchReleases, ...otherProps } =
      this.props;

    return <InteractiveSearch {...otherProps} />;
  }
}
