...
class AddNewItem extends Component {
  ...
  render() {
    const { error, items, hasExistingArtists } = this.props;
                ...
                if (item.artist) {
                  ...
                  return (
                    <AddNewAlbumSearchResultConnector
                      ...
                      isExistingArtist={
                        'id' in album.artist && album.artist.id !== 0
                      }
                      {...album}
                    />
                  );
          ...
          {!isFetching && !error && !items.length && !!term && (
            <div className={styles.message}>
              ...
              <div>
                ...
                <Link to="https://musicbrainz.org/search">
                  {' '}
                  MusicBrainz ID{' '}
                </Link>
                of an artist or release group e.g.
                lidarr:cc197bad-dc9c-440d-a5b5-d52ba2e14234{' '}
                {/* TODO: Update 'lidarr:' search prefix in backend SkyHookProxy.cs first */}
              </div>
            </div>
          )}
          ...
          {!term && !hasExistingArtists ? (
            <div className={styles.message}>
              <div className={styles.noArtistsText}>
                You haven't added any artists yet, do you want to add an
                existing library location (Root Folder) and update?
              </div>
              <div>
                <Button to="/settings/mediamanagement" kind={kinds.PRIMARY}>
                  Add Root Folder
                </Button>
              </div>
            </div>
