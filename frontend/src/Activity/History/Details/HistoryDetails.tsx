...
function HistoryDetails(props) {
  ...
  if (eventType === 'trackFileImported') {
    const { customFormatScore, droppedPath, importedPath } = data;
  ...
  if (eventType === 'trackFileDeleted') {
    const { reason, customFormatScore } = data;
    ...
    switch (reason) {
      ...
      case 'MissingFromDisk':
        reasonMessage =
          'Melodarr was unable to find the file on disk so the file was unlinked from the album/track in the database';
        break;
      case 'Upgrade':
  ...
  if (eventType === 'trackFileRetagged') {
    const { diff, tagsScrubbed } = data;

    return (
      <DescriptionList>
        <DescriptionListItem title={translate('Path')} data={sourceTitle} />
