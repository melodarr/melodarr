...
export const filterPredicates = {
  ...
  sizeOnDisk: function (item, filterValue, type) {
    const predicate = filterTypePredicates[type];
    const sizeOnDisk =
      item.statistics && item.statistics.sizeOnDisk
...
export const persistState = ['artist.deleteOptions'];
...
export const actionHandlers = handleThunks({
  ...
  [SAVE_ARTIST]: createSaveProviderHandler(section, '/artist', {
  ...
  [TOGGLE_ARTIST_MONITORED]: (getState, payload, dispatch) => {
    const { artistId: id, monitored } = payload;
  ...
  [TOGGLE_ALBUM_MONITORED]: function (getState, payload, dispatch) {
    const { artistId: id, seasonNumber, monitored } = payload;
    ...
    promise.done((data) => {
      const albums = _.filter(getState().albums.items, {
        artistId: id,
        seasonNumber,
      });
