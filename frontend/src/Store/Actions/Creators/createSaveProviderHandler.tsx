...
export function createCancelSaveProviderHandler(section) {
  return function (_getState, _payload, _dispatch) {
...
function createSaveProviderHandler(
  ...
  return function (getState, payload, dispatch) {
    ...
    const saveData = Array.isArray(id)
      ? id.map((x) =>
          getProviderState({ id: x, ...otherPayload }, getState, section)
        )
      : getProviderState({ id, ...otherPayload }, getState, section);
    ...
    request.done((data) => {
      ...
      const toRemove =
        removeStale && Array.isArray(id)
          ? _.difference(id, _.map(data, 'id'))
          : [];

      dispatch(
        batchActions(
          data
            .map((item) => updateItem({ section, ...item }))
            .concat(toRemove.map((item) => removeItem({ section, id: item })))
            .concat(
              set({
                section,
                isSaving: false,
