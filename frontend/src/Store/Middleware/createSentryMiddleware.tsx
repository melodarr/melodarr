...
function stripUrlBase(frame) {
  if (frame.filename && window.Melodarr.urlBase) {
    frame.filename = frame.filename.replace(window.Melodarr.urlBase, '');
  }
  return frame;
}
...
export default function createSentryMiddleware() {
  ...
  const dsn = isProduction
    ? 'https://9df0cbe9eedf4b8698e8bc94ec03c4ff@sentry.servarr.com/18'
