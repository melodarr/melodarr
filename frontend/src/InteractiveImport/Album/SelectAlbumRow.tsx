...
import styles from './SelectAlbumRow.module.css';
...
class SelectAlbumRow extends Component {
  ...
  render() {
    ...
    const extendedTitle = disambiguation
      ? `${title} (${disambiguation})`
      : title;

    return (
      <TableRow onClick={this.onPress} className={styles.albumRow}>
        {columns.map((column) => {
          ...
          if (name === 'title') {
            return <TableRowCell key={name}>{extendedTitle}</TableRowCell>;
          }

          if (name === 'albumType') {
            return <TableRowCell key={name}>{albumType}</TableRowCell>;
          }

          if (name === 'releaseDate') {
            return <RelativeDateCellConnector key={name} date={releaseDate} />;
          }
              ...
              <TableRowCell key={name}>
                <Label
                  title={translate(
                    ...
                    [totalTrackCount, trackFileCount]
                  )}
                  kind={getTrackCountKind(
                    monitored,
                    releaseDate,
                    trackFileCount,
                    trackCount
