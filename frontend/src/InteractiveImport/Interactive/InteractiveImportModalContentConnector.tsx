...
class InteractiveImportModalContentConnector extends Component {
  ...
  componentDidMount() {
    const { artistId, downloadId, folder } = this.props;

    const { filterExistingFiles, replaceExistingFiles } = this.state;
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.filterExistingFiles !== filterExistingFiles ||
  ...
  onImportSelectedPress = (selected, importMode) => {
    ...
    if (importMode === 'chooseImportMode') {
      this.setState({
        interactiveImportErrorMessage: 'An import mode must be selected',
      });
      return;
    }
    ...
    _.forEach(this.props.items, (item) => {
      ...
      if (isSelected) {
        ...
        if (!artist) {
          this.setState({
            interactiveImportErrorMessage:
              'Artist must be chosen for each selected file',
          });
          return false;
        }
        ...
        if (!album) {
          this.setState({
            interactiveImportErrorMessage:
              'Album must be chosen for each selected file',
          });
          return false;
        }
        ...
        if (!tracks || !tracks.length) {
          this.setState({
            interactiveImportErrorMessage:
              'One or more tracks must be chosen for each selected file',
          });
          return false;
        }
