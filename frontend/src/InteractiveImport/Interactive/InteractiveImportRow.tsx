...
class InteractiveImportRow extends Component {
  ...
  componentDidMount() {
    const { id, artist, album, tracks, quality, size } = this.props;

    if (artist && album != null && tracks.length && quality && size > 0) {
      this.props.onSelectedChange({ id, value: true });
    }
  }

  componentDidUpdate(prevProps) {
    const { id, artist, album, tracks, quality, isSelected, onValidRowChange } =
      this.props;
    ...
    const isValid = !!(artist && album && tracks.length && quality);
  ...
  render() {
    ...
    let albumTitle = '';
    if (album) {
      albumTitle = album.disambiguation
    ...
    const sortedTracks = tracks.sort(
      (a, b) =>
        parseInt(a.absoluteTrackNumber) - parseInt(b.absoluteTrackNumber)
    );

    const trackNumbers = sortedTracks
      .map((track) => `${track.mediumNumber}x${track.trackNumber}`)
      .join(', ');

    const showArtistPlaceholder = isSelected && !artist;
    const showAlbumNumberPlaceholder = isSelected && !!artist && !album;
    const showTrackNumbersPlaceholder =
      !isReprocessing && isSelected && !!album && !tracks.length;
    ...
    const pathCellContents = <div>{path}</div>;
    ...
    const isIndexerFlagsColumnVisible =
      columns.find((c) => c.name === 'indexerFlags')?.isVisible ?? false;

    return (
      <TableRow className={additionalFile ? styles.additionalFile : undefined}>
        <TableSelectCell
          id={id}
        ...
        <TableRowCell className={styles.path} title={path}>
          {pathCell}
        </TableRowCell>
        ...
        <TableRowCellButton
          ...
          title={
            allowArtistChange
              ? translate('AllowArtistChangeClickToChangeArtist')
              : undefined
          }
        ...
        <TableRowCellButton
          ...
          title={
            artist && album
              ? translate('ArtistAlbumClickToChangeTrack')
              : undefined
          }
          onPress={this.onSelectTrackPress}
        >
          {showTrackNumbersLoading && (
            <LoadingIndicator size={20} className={styles.loading} />
          )}
          {showTrackNumbersPlaceholder ? (
            <InteractiveImportRowCellPlaceholder />
          ) : (
            trackNumbers
          )}
