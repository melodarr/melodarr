...
const importModeOptions = [
  {
    ...
    value: () => translate('ChooseImportMethod'),
    disabled: true,
  },
...
class InteractiveImportModalContent extends Component {
  ...
  componentDidUpdate(_prevProps) {
    ...
    const selectedItems = _.filter(this.props.items, (x) =>
      _.includes(selectedIds, x.id)
    );
    ...
    const inconsistent = _(selectedItems)
      .map((x) => ({
        albumId: x.album ? x.album.id : 0,
        releaseId: x.albumReleaseId,
      }))
      .groupBy('albumId')
      .mapValues(
        (album) =>
  ...
  onValidRowChange = (id, isValid) => {
    this.setState((state, props) => {
      ...
      const diff = _.difference(
        state.invalidRowsSelected,
        _.map(props.items, 'id')
      );
      const currentInvalid = _.difference(state.invalidRowsSelected, diff);
      const newstate = isValid
        ? _.without(currentInvalid, id)
        : _.union(currentInvalid, [id]);
  ...
  onConfirmImportPress = () => {
    const { downloadId, showImportMode, importMode, onImportSelectedPress } =
      this.props;
  ...
  onFilterExistingFilesChange = (value) => {
    this.props.onFilterExistingFilesChange(
      value !== filterExistingFilesOptions.ALL
    );
  };

  onReplaceExistingFilesChange = (value) => {
    this.props.onReplaceExistingFilesChange(
      value === replaceExistingFilesOptions.DELETE
    );
  };
  ...
  render() {
    ...
    const columns = allColumns.map((column) => {
      ...
      if (!showIndexerFlags) {
        const indexerFlagsColumn = allColumns.find(
          (c) => c.name === 'indexerFlags'
        );
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
        ...
        <SelectAlbumModal
          ...
          ids={selectedIds}
          artistId={
            selectedItem && selectedItem.artist && selectedItem.artist.id
          }
          onModalClose={this.onSelectModalClose}
        />
        ...
        <SelectAlbumReleaseModal
          ...
          importIdsByAlbum={_.chain(items)
            .filter((x) => x.album)
            .groupBy((x) => x.album.id)
            .mapValues((x) => x.map((y) => y.id))
            .value()}
          albums={_.chain(items)
            .filter((x) => x.album)
            .keyBy((x) => x.album.id)
            .mapValues((x) => ({
              matchedReleaseId: x.albumReleaseId,
              album: x.album,
            }))
            .values()
            .value()}
