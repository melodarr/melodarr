...
import styles from './SelectTrackModalContent.module.css';
...
class SelectTrackModalContent extends Component {
  ...
  constructor(props, context) {
    ...
    const selectedTracks = _.filter(props.selectedTracksByItem, [
      'id',
      props.id,
    ])[0].tracks;
    const init = _.zipObject(
      selectedTracks,
      _.times(selectedTracks.length, _.constant(true))
    );
  ...
  render() {
    ...
    const { allSelected, allUnselected, filter, selectedState } = this.state;
    ...
    const otherSelected = _.map(
      _.filter(selectedTracksByItem, (item) => {
        return item.id !== id;
      }),
      (x) => {
        return x.tracks;
    ...
    const currentSelected = _.keys(_.pickBy(selectedState, _.identity)).map(
      Number
    );
    ...
    return (
      <ModalContent onModalClose={onModalClose}>
            ...
            {isPopulated && !!items.length ? (
              <Table
                columns={
                  selectAllEnabled
                    ? columns
                    : selectAllBlankColumn.concat(columns)
                }
        ...
        <ModalFooter>
          ...
          <Button kind={kinds.SUCCESS} onPress={this.onTracksSelect}>
