...
class AlbumDetailsPageConnector extends Component {
  ...
  render() {
    const { foreignAlbumId, isFetching, isPopulated } = this.props;

    if (!foreignAlbumId) {
      return <NotFound message={translate('SorryThatAlbumCannotBeFound')} />;
    }
    ...
    if (!isFetching && isPopulated && this.state.hasMounted) {
      return <AlbumDetailsConnector foreignAlbumId={foreignAlbumId} />;
    }
  }
}
...
AlbumDetailsPageConnector.propTypes = {
  ...
  match: PropTypes.shape({
    params: PropTypes.shape({ foreignAlbumId: PropTypes.string.isRequired })
      .isRequired,
  }).isRequired,
