...
function createMapStateToProps() {
  return createSelector(
    ...
    (
      ...
      const previousAlbum =
        sortedAlbums[albumIndex - 1] || _.last(sortedAlbums);
      const nextAlbum = sortedAlbums[albumIndex + 1] || _.first(sortedAlbums);
      const isSearchingCommand = findCommand(commands, {
...
class AlbumDetailsConnector extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { id, anyReleaseOk, isRenamingFiles, isRenamingArtist } = this.props;

    if (
      (prevProps.isRenamingFiles && !isRenamingFiles) ||
      (prevProps.isRenamingArtist && !isRenamingArtist) ||
      !_.isEqual(
        getMonitoredReleases(prevProps),
        getMonitoredReleases(this.props)
      ) ||
