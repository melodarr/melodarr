...
function EpisodeStatus(props) {
  const { releaseDate, monitored, grabbed, queueItem, trackFile } = props;
    ...
    return (
      <div className={styles.center}>
        <QueueDetails
          ...
          progressBar={
            <ProgressBar
              title={translate('AlbumIsDownloadingInterp', [
                progress.toFixed(1),
                queueItem.title,
              ])}
              progress={progress}
  ...
  if (isReleased) {
    return (
      <div className={styles.center}>
        <Icon name={icons.MISSING} title={translate('TrackMissingFromDisk')} />
      </div>
    );
  }
  ...
  return (
    <div className={styles.center}>
      <Icon name={icons.NOT_AIRED} title={translate('AlbumHasNotAired')} />
    </div>
  );
}
