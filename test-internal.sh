...
# If we are running from source repo, test the SLN directly
if [ ! -f "Lidarr.Api.Test.dll" ] && [ ! -f "Melodarr.Api.Test.dll" ] && [ -f "src/Melodarr.sln" ]; then
    DOTNET_PARAMS="src/Melodarr.sln --filter:$WHERE $VSTEST_PARAMS"
else
    # Otherwise we are in the packaged CI test folder, test explicitly provided dlls
    for i in "${FILES[@]}";
